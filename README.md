...
| 按住不放 | 连发。空格键同理 |
| 同频 | 两人 3 秒内先后点击，两边一起爆开金色，背景的小爱心也跟着变金色 |
| 对方不在时 | 你点的都攒着，等 TA 上线一次放出 |
| 小纸条 | 快捷词或自由输入，飘过两边屏幕。服务端留存，`/debug` 调试台可翻 |
| 同在时的音乐 | 两个人都在，`music/` 里的歌随机放；人一走淡出。两边同一首、同一处 |
| 选歌 | 顶栏「选歌」或点正在放的那首，列出全部曲目。挑一首两边一起换 |
| 正在放什么 | ♪ 左边显示曲名和时间，进度是那块底色从左往右铺过去 |
| 只关自己这边 | 右上角 ♫ 只管自己的耳朵，TA 那边照放；再打开就接回当前那一处 |
| 背景小爱心 | 一直缓缓往上冒。一个人时只有你的粉色，TA 也在就一半粉一半蓝 |
...
**两个人听到的是同一首、同一处。** 放哪首、什么时候开的头，只有服务端那一份说了算
（`server.js` 的「同一首歌，同一处」）：页面拿到的是「这首已经放了多少毫秒」，
收到就以自己的钟为准往下走，不用两边对表，差的只是一趟网络的工夫。
中途进来的直接接到当前那一处，不用等下一首。

**选歌**：顶栏「选歌」按钮，或者点正在放的那一格，会列出 `music/` 里的全部曲目。
点一首，两个人一起从这首的开头听；「换一首」是随机再抓一首。
一个人的时候没有音乐这回事，清单里点不动，服务端也不认。

**右上角 ♫ 只关自己这边的耳朵**，TA 那边照放不误；再打开就接回当前那一处。
这跟「音乐停了」是两回事 —— 停不停由「两个人在不在」决定。
...
♫ 会提示「点一下心跳，音乐就接上」，点一下大爱心就接上了。

Chrome 还有一条：**页面藏在后台标签页时，它压根不去下载新的一首**，
所以那边会停在曲名上不动，切回来才开始响 —— 切回来的那一下会直接落到
TA 正听着的那一处，不会从头放。手机锁屏、切走 App 同理。
...
```
{"t":1785123319008,"name":"chris","text":"miss you"}
```
...
```
260727 14:26:01  chris  miss you
```
...
# {"t":1785123319008,"time":"260727 11:35:19","name":"chris","text":"miss you"}
```
